...
function gt3_get_product_name (){
    $product = 'Oconnor – Lawyers Attorneys and Law Firm WordPress Theme';
    return $product;
}
...
function gt3_account_activation($code = '',$email = ''){
    ...
    if (!empty($email)) {
        ...
        if ( $response_code != 200 || is_wp_error( $version_info ) ) {
            $errore_return = new WP_Error( 'registration-error-connection-error', esc_html__( 'Registration Connection error', 'oconnor' ) );
            return json_encode($errore_return);
        }
    }else{
        $errore_return = new WP_Error( 'registration-error-invalid-email', esc_html__( 'Please provide a valid email address.', 'oconnor' ) );
        return json_encode($errore_return);
    }
...
if (get_option( 'gt3_registration_status') != 'active' && class_exists( 'GT3_oconnor_core' ) && get_option( 'gt3_supported_account_registration_srart') != 'false') {
...
update_option( 'gt3_supported_account_registration_srart', 'true' );
function gt3_support_notice(){
    if (get_option('gt3_supported_notice_srart') == 'true' && class_exists( 'GT3_oconnor_core' )) {
...
function gt3_registration_notice() {
    if (get_option( 'gt3_supported_account_registration_srart') != 'false') {
      ?>
      <div class="notice notice-error is-dismissible" style="padding: 5px 25px 15px 75px;position: relative;">
        ...
        <p><strong><?php esc_html_e( 'Purchase Validation! Please activate your theme.', 'oconnor' ); ?></strong></p>
        <div><a class="button button-primary" href="admin.php?page=<?php echo is_child_theme() == true ? 'OConnor-Child' : 'OConnor' ?>&tab=18"><?php esc_html_e( 'Register Now', 'oconnor' ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a> <a target="_blank" class="button button-primary" href="http://themeforest.net/cart/add_items?item_ids=22089634&ref=gt3themes"><?php esc_html_e( 'Buy Theme', 'oconnor' ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a> <a style="vertical-align: bottom;margin-left: 10px;line-height: 1.6;" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'gt3-hide-notice', 'account_registration_srart' ), 'gt3_hide_notices_nonce', '_gt3_notice_nonce' ) ); ?>"><?php esc_html_e( 'Dismiss this notice', 'oconnor' ); ?></a></div>
      </div>
      <?php
    }
}
...
function gt3_registration_notice_supported_until() {
  ...
  ?>
    <div class="notice notice-error is-dismissible" style="padding: 10px 25px 10px 75px;position: relative;">
        <i class="fa fa-exclamation" aria-hidden="true" style="position: absolute; top: 50%; left: 15px; margin-top: -22px;font-size: 25px; line-height: 40px; width: 40px;text-align: center; border: 2px solid;border-radius: 40px;"></i>
        ...
        if (!empty($support_time_left['expired']) && $support_time_left['expired'] == true) {
            esc_html_e( 'Your support package for this theme expired', 'oconnor' ); ?><?php echo " ( ".$support_time_left['time_to_left']." ".esc_html__( 'ago', 'oconnor' )." ).";
        }else{
            esc_html_e( 'Your support package for this theme is about to expire', 'oconnor' ); ?><?php echo " ( ".$support_time_left['time_to_left']." ".esc_html( 'left', 'oconnor' )." ).";
        }
        ?></p>
        <div style="margin-bottom: 10px;"><a class="button button-primary" href="<?php echo esc_url($theme_link); ?>" style="margin-top: 5px;"><?php esc_html_e( 'Update Support Package', 'oconnor' ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a> <a class="button button-primary" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'gt3-hide-notice', 'notice_srart' ), 'gt3_hide_notices_nonce', '_gt3_notice_nonce' ) ); ?>" style="margin-top: 5px;"><?php esc_html_e( 'Dismiss this notice', 'oconnor' ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a></div>
    </div>
  <?php
    }
}
...
function gt3_hide_hotice (){
    if ( isset( $_GET['gt3-hide-notice'] ) && isset( $_GET['_gt3_notice_nonce'] ) ) {
        if ( ! wp_verify_nonce( $_GET['_gt3_notice_nonce'], 'gt3_hide_notices_nonce' ) ) {
            wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'oconnor' ) );
        }
...
function gt3_account_notice(){
  ...
  <div class="notice notice-warning" style="padding: 10px">
    <p><strong><?php esc_html_e( 'Create account and manage your theme purchase codes.', 'oconnor' ) ?></strong></p>
    <p><strong><a href="admin.php?page=<?php echo is_child_theme() == true ? 'OConnor-Child' : 'OConnor' ?>&tab=21"><?php esc_html_e( 'Register Now', 'oconnor' ); ?></a> |  <a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'gt3-hide-notice', 'account_notice_srart' ), 'gt3_hide_notices_nonce', '_gt3_notice_nonce' ) ); ?>"><?php esc_html_e( 'Dismiss this notice', 'oconnor' ); ?></a></strong></p>
  </div>
  <?php
}
