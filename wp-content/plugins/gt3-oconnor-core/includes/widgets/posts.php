...
class posts extends WP_Widget {
    ...
    function widget($args, $instance) {
        ...
        while ($gt3_wp_query_posts->have_posts()) {
            $gt3_wp_query_posts->the_post();
            ...
            if (empty($gt3_theme_featured_image_latest)) {
                ...
                $widg_img = '<a href="' . esc_url(get_permalink()) . '"><img src="' . esc_url(aq_resize($gt3_theme_featured_image_latest[0], "140", "140", true, true, true)) . '" alt="' . get_the_title() . '"></a>';
            }
            ...
            if (!empty($instance['posts_widget_date_hide']) && $instance['posts_widget_date_hide'] == 'on') {
                $post_date = '<div class="listing_meta"><span>' . get_the_time(get_option('date_format')) . '</span></div>';
            } else {
                $post_date    = '';
                $widget_class .= ' no_date';
            }
    ...
    function update($new_instance, $old_instance) {
        ...
        $instance['posts_widget_date_hide']    = $new_instance['posts_widget_date_hide'];
    ...
    function form($instance) {
        $defaultValues = array(
            'widget_title'              => esc_html__('Recent Posts', 'oconnor'),
            'posts_widget_number'       => '3',
            'posts_widget_content_hide' => '',
            'posts_widget_date_hide'    => ''
        );
        ...
        $posts_widget_date_hide = isset($instance['posts_widget_date_hide']) ? (bool)$instance['posts_widget_date_hide'] : false;
        ?>
        <table class="fullwidth">
            <tr>
                <td><?php echo esc_html__('Title:', 'oconnor'); ?></td>
            ...
            <tr>
                <td><?php echo esc_html__('Number:', 'oconnor'); ?></td>
                <td><input type='text' class="fullwidth"
                           name='<?php echo esc_attr($this->get_field_name('posts_widget_number')); ?>'
                           value='<?php echo esc_attr($instance['posts_widget_number']); ?>'/></td>
            </tr>
            <tr>
                <td><?php echo esc_html__('Show post content:', 'oconnor'); ?></td>
                <td><input type='checkbox' class="checkbox"
                           id='<?php echo esc_attr($this->get_field_id('posts_widget_content_hide')); ?>'
                           name='<?php echo esc_attr($this->get_field_name('posts_widget_content_hide')); ?>'
                        <?php checked($posts_widget_content_hide); ?> /></td>
            </tr>
            <tr>
                <td><?php echo esc_html__('Show post date:', 'oconnor'); ?></td>
                <td><input type='checkbox' class="checkbox"
                           id='<?php echo esc_attr($this->get_field_id('posts_widget_date_hide')); ?>'
                           name='<?php echo esc_attr($this->get_field_name('posts_widget_date_hide')); ?>'
                        <?php checked($posts_widget_date_hide); ?> /></td>
            </tr>
        </table>
        <?php
    }
