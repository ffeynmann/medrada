    ...
    class ReduxFramework_gt3_presets {
        ...
        public function get_buttons($x) {
            echo "<div class='gt3_presets__operation'>";
                echo "<div class='gt3_presets__operation_item operation_clone' title='".esc_html__('Clone','gt3_oconnor_core')."'><i class='fa fa-clone' aria-hidden='true'></i></div>";
                if ($x != 0) {                
                    echo "<div class='gt3_presets__operation_item operation_edit' title='".esc_html__('Edit','gt3_oconnor_core')."'><i class='fa fa-pencil' aria-hidden='true'></i></div>";
                    echo "<div class='gt3_presets__operation_item operation_remove' title='".esc_html__('Remove','gt3_oconnor_core')."'><i class='fa fa-times' aria-hidden='true'></i></div>";
                }
            echo "</div>";
        ...
        public function render() {
            ...
            if ( ! empty( $this->field['options'] ) || ! empty($this->field['default']) ) {
                ...
                echo '<div class="redux-gt3_preset-accordion" data-new-content-title="'.esc_attr__('New Preset','gt3_oconnor_core').'">';
                ...
                if ( isset ( $this->value ) && 
                    ...
                    foreach ($val as $key => $item) {
                        echo '<div class="gt3_preset__wrapper'.($current_active == $key ? ' active' : '').'">';
                            echo '<fieldset class="redux-field" data-id="' . $this->field[ 'id' ] . '">';
                                $hide = ' hide';
                                $title_option = !empty($item) && !empty($item['title']) ? $item['title'] : esc_attr__('Preset','gt3_oconnor_core') . ' - '. $key; 
                                ...
                                /*if ($key == 0) {
                                    if (empty($preset_option['gt3_header_builder_id'])) {
                                        $preset_option = array('gt3_header_builder_id' => $preset_option );
                                    }
                                }*/
                    ...
                    echo '<div class="gt3_preset__wrapper">';
                        echo '<fieldset class="redux-field" data-id="' . $this->field[ 'id' ] . '">';
                            /*echo '<h3><span class="gt3_preset__item_header">'.esc_attr__('New Preset','gt3_oconnor_core').'</span></h3>';*/
                            $hide = ' hide';
                            $def_option = $this->field['default'];
                            $def_option = !empty($def_option[0]) ? $def_option[0] : array();
                            $title_option = !empty($def_option) && !empty($def_option['title']) ? $def_option['title'] : esc_attr__('Default','gt3_oconnor_core'); 
                ...
                echo "<div class='gt3_preset_add_new'>";
                    echo "<div class='gt3_preset_add_new_button'>";
                        echo esc_html__('Add New','gt3_oconnor_core');
                    echo "</div>";
                echo "</div>";
                // End Add New
                ...
                echo "<div class='gt3_preset_add_new__template active default'>";
                    ...
                    echo "<div class='gt3_preset_add_new__template_holder' data-preset='".htmlspecialchars( json_encode( $preset_option ), ENT_QUOTES, 'UTF-8' )."'>".esc_html__('Default','gt3_oconnor_core')."</div>";
                echo "</div>";

                echo "<div class='gt3_preset_add_new__submit'>";
                    echo "<input type='text' placeholder='".esc_html__('Name','gt3_oconnor_core')."' class='gt3_preset_add_new__submit_name' value='".esc_html__('New Preset','gt3_oconnor_core')."'>";
                    echo "<div class='gt3_preset_add_new__submit_button'>".esc_html__('Submit','gt3_oconnor_core')."</div>";
                echo "</div>";
