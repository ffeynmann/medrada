...
abstract class DOM
{
    ...
    public static function block_tags($reset = \false)
    {
        if (empty(self::$block_tags) || $reset) {
            self::$block_tags = \array_merge(\array_flip(\array_filter(\array_keys(\WP_Typography\Vendor\Masterminds\HTML5\Elements::$html5), function ($tag) {
                return \WP_Typography\Vendor\Masterminds\HTML5\Elements::isA($tag, \WP_Typography\Vendor\Masterminds\HTML5\Elements::BLOCK_TAG);
            })), \array_flip(['li', 'td', 'dt']));
        }
    ...
    public static function inappropriate_tags($reset = \false)
    {
        if (empty(self::$inappropriate_tags) || $reset) {
            self::$inappropriate_tags = \array_flip(\array_merge(\array_filter(\array_keys(\WP_Typography\Vendor\Masterminds\HTML5\Elements::$html5), function ($tag) {
                return \WP_Typography\Vendor\Masterminds\HTML5\Elements::isA($tag, \WP_Typography\Vendor\Masterminds\HTML5\Elements::VOID_TAG) || \WP_Typography\Vendor\Masterminds\HTML5\Elements::isA($tag, \WP_Typography\Vendor\Masterminds\HTML5\Elements::TEXT_RAW) || \WP_Typography\Vendor\Masterminds\HTML5\Elements::isA($tag, \WP_Typography\Vendor\Masterminds\HTML5\Elements::TEXT_RCDATA);
    ...
    private static function get_adjacent_chr(\DOMNode $node, $position, $length, callable $get_textnode)
    {
        ...
        if (isset($textnode) && isset($textnode->data)) {
            // Determine encoding.
            $func = \PHP_Typography\Strings::functions($textnode->data);
            if (!empty($func)) {
