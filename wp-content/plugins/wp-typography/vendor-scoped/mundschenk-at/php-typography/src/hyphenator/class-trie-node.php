...
final class Trie_Node
{
    ...
    public function get_node($char)
    {
        if (!isset($this->links[$char])) {
            $this->links[$char] = new \PHP_Typography\Hyphenator\Trie_Node();
        }
    ...
    {
        $trie = new \PHP_Typography\Hyphenator\Trie_Node();
        foreach ($patterns as $key => $pattern) {
            $node = $trie;
            foreach (\PHP_Typography\Strings::mb_str_split($key) as $char) {
