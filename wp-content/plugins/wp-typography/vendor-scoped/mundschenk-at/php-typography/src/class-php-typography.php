...
class PHP_Typography
{
     ...
     */
    public function __construct(\PHP_Typography\Fixes\Registry $registry = null)
    {
     ...
     */
    public function process($html, \PHP_Typography\Settings $settings, $is_title = \false, array $body_classes = [])
    {
     ...
     * @return string The processed $html.
     */
    public function process_feed($html, \PHP_Typography\Settings $settings, $is_title = \false, array $body_classes = [])
    {
        return $this->process_textnodes($html, function ($html, $settings, $is_title) {
     ...
     * @return string The processed $html.
     */
    public function process_textnodes($html, callable $fixer, \PHP_Typography\Settings $settings, $is_title = \false, array $body_classes = [])
    {
        if (isset($settings['ignoreTags']) && $is_title && (\in_array(
            'h1',
        ...
        foreach ($xpath->query('//text()', $body_node) as $textnode) {
            if (self::arrays_intersect(\PHP_Typography\DOM::get_ancestors($textnode), $tags_to_ignore) || $textnode->isWhitespaceInElementContent()) {
                continue;
            }
     ...
     */
    public function parse_html(\WP_Typography\Vendor\Masterminds\HTML5 $parser, $html, \PHP_Typography\Settings $settings, array $body_classes = [])
    {
        ...
        if (!empty($settings[\PHP_Typography\Settings::PARSER_ERRORS_HANDLER]) && !empty($errors)) {
            $errors = $settings[\PHP_Typography\Settings::PARSER_ERRORS_HANDLER]($errors);
        }
        // Return null if there are still unhandled parsing errors.
        if (!empty($errors) && !$settings[\PHP_Typography\Settings::PARSER_ERRORS_IGNORE]) {
            $dom = null;
        }
        return $dom;
    }
     ...
     */
    public function query_tags_to_ignore(\DOMXPath $xpath, \DOMNode $initial_node, \PHP_Typography\Settings $settings)
    {
        $elements = [];
        $query_parts = [];
        ...
        if (!empty($settings['ignoreClasses'])) {
            $query_parts[] = "//*[contains(concat(' ', @class, ' '), ' " . \implode(
                " ') or contains(concat(' ', @class, ' '), ' ",
                /** Array. @scrutinizer ignore-type */
                $settings['ignoreClasses']
            ) . " ')]";
        }
        if (!empty($settings['ignoreIDs'])) {
            $query_parts[] = '//*[@id=\'' . \implode(
                '\' or @id=\'',
                /** Array. @scrutinizer ignore-type */
                $settings['ignoreIDs']
            ) . '\']';
        ...
        if (!empty($query_parts)) {
            ...
            if (\false !== $nodelist) {
                $elements = \PHP_Typography\DOM::nodelist_to_array($nodelist);
            }
        }
        return $elements;
    }
    ...
    public function replace_node_with_html(\DOMNode $node, $content)
    {
        ...
        $content = \PHP_Typography\RE::unescape_tags(\htmlspecialchars($content, \ENT_NOQUOTES | \ENT_HTML5 | \ENT_SUBSTITUTE, 'UTF-8', \true));
        \set_error_handler([$this, 'handle_parsing_errors']);
        // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_set_error_handler
        ...
        if (!empty($html_fragment)) {
            ...
            if (!empty($imported_fragment)) {
                ...
                $children = \PHP_Typography\DOM::nodelist_to_array($imported_fragment->childNodes);
    ...
    public function get_registry()
    {
        if (!isset($this->registry)) {
            $this->registry = new \PHP_Typography\Fixes\Default_Registry($this->get_hyphenator_cache());
    ...
    public function get_hyphenator_cache()
    {
        if (!isset($this->hyphenator_cache)) {
            $this->hyphenator_cache = new \PHP_Typography\Hyphenator\Cache();
        }
     ...
     */
    public function set_hyphenator_cache(\PHP_Typography\Hyphenator\Cache $cache)
    {
