...
class Text_Parser
{
    ...
    const _SPACE = '(?:\\s|' . self::_HTML_SPACING . ')+';
    ...
    // Find any text.
    const _ANY_TEXT = self::_SPACE . '|' . self::_PUNCTUATION . '|' . self::_WORD;
    // required modifiers: x (multiline pattern) i (case insensitive) u (utf8).
    // Regular expressions.
    ...
    public function load($raw_text)
    {
        ...
        // Detect encoding.
        $str_functions = \PHP_Typography\Strings::functions($raw_text);
        ...
        $this->text = self::tokenize(
            /** RE correct. @scrutinizer ignore-type */
            \preg_split(self::_RE_ANY_TEXT, $raw_text, -1, \PREG_SPLIT_DELIM_CAPTURE | \PREG_SPLIT_NO_EMPTY)
        );
    ...
    protected static function tokenize(array $parts)
    {
        ...
        foreach ($parts as $part) {
            if (\preg_match(self::_RE_SPACE, $part)) {
                $tokens[$index] = new \PHP_Typography\Text_Parser\Token($part, \PHP_Typography\Text_Parser\Token::SPACE);
            } elseif (\preg_match(self::_RE_PUNCTUATION, $part)) {
                $tokens[$index] = new \PHP_Typography\Text_Parser\Token($part, \PHP_Typography\Text_Parser\Token::PUNCTUATION);
                ...
                self::parse_ambiguous_token(\PHP_Typography\Text_Parser\Token::WORD, $part, $tokens, $index);
            } else {
                ...
                self::parse_ambiguous_token(\PHP_Typography\Text_Parser\Token::OTHER, $part, $tokens, $index);
            }
            $index++;
        }
        return $tokens;
    ...
    protected static function parse_ambiguous_token($expected_type, $part, array &$tokens, &$index)
    {
        ...
        if (self::is_preceeded_by(\PHP_Typography\Text_Parser\Token::OTHER, $tokens, $index) || \PHP_Typography\Text_Parser\Token::OTHER === $expected_type && self::is_preceeded_by(\PHP_Typography\Text_Parser\Token::WORD, $tokens, $index)) {
            $index--;
            $old_part = $tokens[$index]->value;
            $tokens[$index] = new \PHP_Typography\Text_Parser\Token($old_part . $part, \PHP_Typography\Text_Parser\Token::OTHER);
        } elseif (self::is_preceeded_by(\PHP_Typography\Text_Parser\Token::PUNCTUATION, $tokens, $index) && self::is_not_preceeded_by(\PHP_Typography\Text_Parser\Token::SPACE, $tokens, $index, 2)) {
            // Not preceeded by a non-space + punctuation.
            $old_part = $tokens[$index - 1]->value;
            $older_part = $tokens[$index - 2]->value;
            $tokens[$index - 2] = new \PHP_Typography\Text_Parser\Token($older_part . $old_part . $part, \PHP_Typography\Text_Parser\Token::OTHER);
            unset($tokens[$index - 1]);
            $index = $index - 2;
        } else {
            // All good.
            $tokens[$index] = new \PHP_Typography\Text_Parser\Token($part, $expected_type);
        }
    }
    /**
    ...
    {
        return $this->get_type(\PHP_Typography\Text_Parser\Token::SPACE);
    }
    /**
    ...
    {
        return $this->get_type(\PHP_Typography\Text_Parser\Token::PUNCTUATION);
    }
    /**
    ...
    public function get_words($abc = self::ALLOW_ALL_LETTERS, $caps = self::ALLOW_ALL_CAPS, $comps = self::ALLOW_COMPOUNDS)
    {
        ...
        // Result set.
        $tokens = [];
        foreach ($this->get_type(\PHP_Typography\Text_Parser\Token::WORD) as $index => $token) {
     ...
     * @return bool
     */
    protected function conforms_to_letters_policy(\PHP_Typography\Text_Parser\Token $token, $policy)
    {
     ...
     * @return bool
     */
    protected function conforms_to_caps_policy(\PHP_Typography\Text_Parser\Token $token, $policy)
    {
     ...
     * @return bool
     */
    protected function conforms_to_compounds_policy(\PHP_Typography\Text_Parser\Token $token, $policy)
    {
     ...
     * @return bool
     */
    protected function check_policy(\PHP_Typography\Text_Parser\Token $token, $policy, $permissive_policy, $equal_policy, $non_equal_policy, callable $transform_token)
    {
        // Short circuit.
    ...
    {
        return $this->get_type(\PHP_Typography\Text_Parser\Token::OTHER);
    }
    /**
