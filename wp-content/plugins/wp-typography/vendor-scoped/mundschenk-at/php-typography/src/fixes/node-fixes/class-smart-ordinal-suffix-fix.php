 ...
 * @since 5.0.0
 */
class Smart_Ordinal_Suffix_Fix extends \PHP_Typography\Fixes\Node_Fixes\Abstract_Node_Fix
{
    // Possible suffixes.
    ...
    const WORD_BOUNDARY_START = '\\b(?<![' . \PHP_Typography\U::SOFT_HYPHEN . \PHP_Typography\U::ZERO_WIDTH_SPACE . '])';
    const WORD_BOUNDARY_END = '\\b(?![' . \PHP_Typography\U::SOFT_HYPHEN . \PHP_Typography\U::ZERO_WIDTH_SPACE . '])';
    /**
    ...
    public function __construct($css_class = null, $feed_compatible = \false)
    {
        ...
        $this->replacement = \PHP_Typography\RE::escape_tags("\$1<sup{$ordinal_class}>\$2</sup>");
    }
    /**
    ...
    {
        if (empty($settings[\PHP_Typography\Settings::SMART_ORDINAL_SUFFIX])) {
            return;
        }
        // Always match Arabic numbers.
        $patterns = [self::RE_ARABIC_ORDINALS];
        // Only match Roman numbers if explicitely enabled.
        if (!empty($settings[\PHP_Typography\Settings::SMART_ORDINAL_SUFFIX_ROMAN_NUMERALS])) {
