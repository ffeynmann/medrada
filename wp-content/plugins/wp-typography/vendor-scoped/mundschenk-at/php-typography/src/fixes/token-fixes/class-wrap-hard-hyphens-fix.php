 ...
 * @since 5.0.0
 */
class Wrap_Hard_Hyphens_Fix extends \PHP_Typography\Fixes\Token_Fixes\Abstract_Token_Fix
{
    /**
    ...
    public function __construct($feed_compatible = \false)
    {
        ...
        $this->hyphens_array = \array_unique(['-', \PHP_Typography\U::HYPHEN]);
        $this->remove_ending_space_regex = '/(' . \implode('|', $this->hyphens_array) . ')' . \PHP_Typography\U::ZERO_WIDTH_SPACE . '$/';
    }
    /**
    ...
    {
        if (!empty($settings[\PHP_Typography\Settings::HYPHEN_HARD_WRAP])) {
            foreach ($tokens as $index => $text_token) {
                $value = $text_token->value;
                if (isset($settings[\PHP_Typography\Settings::HYPHEN_HARD_WRAP]) && $settings[\PHP_Typography\Settings::HYPHEN_HARD_WRAP]) {
                    $value = \str_replace($this->hyphens_array, '-' . \PHP_Typography\U::ZERO_WIDTH_SPACE, $value);
                    $value = \str_replace('_', '_' . \PHP_Typography\U::ZERO_WIDTH_SPACE, $value);
                    $value = \str_replace('/', '/' . \PHP_Typography\U::ZERO_WIDTH_SPACE, $value);
