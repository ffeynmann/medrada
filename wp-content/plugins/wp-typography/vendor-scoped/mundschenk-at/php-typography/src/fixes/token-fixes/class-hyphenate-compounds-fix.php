 ...
 * @since 5.0.0
 */
class Hyphenate_Compounds_Fix extends \PHP_Typography\Fixes\Token_Fixes\Hyphenate_Fix
{
    /**
    ...
    {
        parent::__construct($cache, \PHP_Typography\Fixes\Token_Fix::COMPOUND_WORDS, $feed_compatible);
    }
    /**
    ...
    {
        if (empty($settings[\PHP_Typography\Settings::HYPHENATE_COMPOUNDS])) {
            return $tokens;
            // abort.
        }
        ...
        foreach ($tokens as $key => $word_token) {
            ...
            foreach (\preg_split('/(-)/', $word_token->value, -1, \PREG_SPLIT_NO_EMPTY | \PREG_SPLIT_DELIM_CAPTURE) as $word_part) {
                $component_words[] = new \PHP_Typography\Text_Parser\Token($word_part, \PHP_Typography\Text_Parser\Token::WORD);
            }
