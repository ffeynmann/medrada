 ...
 * @since 5.0.0
 */
class Smart_Quotes_Fix extends \PHP_Typography\Fixes\Node_Fixes\Abstract_Node_Fix
{
    ...
    {
        if (empty($settings[\PHP_Typography\Settings::SMART_QUOTES])) {
            return;
        }
        // Need to get context of adjacent characters outside adjacent inline tags or HTML comment
        // if we have adjacent characters add them to the text.
        $previous_character = \PHP_Typography\DOM::get_prev_chr($textnode);
        $next_character = \PHP_Typography\DOM::get_next_chr($textnode);
        $node_data = "{$previous_character}{$textnode->data}{$next_character}";
        $f = \PHP_Typography\Strings::functions($node_data);
        ...
        $double_open = \PHP_Typography\RE::ESCAPE_MARKER . $double->open() . \PHP_Typography\RE::ESCAPE_MARKER;
        $double_close = \PHP_Typography\RE::ESCAPE_MARKER . $double->close() . \PHP_Typography\RE::ESCAPE_MARKER;
        $single_open = \PHP_Typography\RE::ESCAPE_MARKER . $single->open() . \PHP_Typography\RE::ESCAPE_MARKER;
        $single_close = \PHP_Typography\RE::ESCAPE_MARKER . $single->close() . \PHP_Typography\RE::ESCAPE_MARKER;
        ...
        if (!empty($settings[\PHP_Typography\Settings::SMART_QUOTES_EXCEPTIONS])) {
            $node_data = \str_replace($settings[\PHP_Typography\Settings::SMART_QUOTES_EXCEPTIONS]['patterns'], $settings[\PHP_Typography\Settings::SMART_QUOTES_EXCEPTIONS]['replacements'], $node_data);
        }
        ...
        // Guillemets.
        $node_data = \str_replace(['<<', '>>'], [\PHP_Typography\U::GUILLEMET_OPEN, \PHP_Typography\U::GUILLEMET_CLOSE], $node_data);
        // Primes.
        $node_data = \preg_replace([
            ...
            '$1' . \PHP_Typography\U::SINGLE_PRIME . '$2$3' . \PHP_Typography\U::DOUBLE_PRIME,
            // @codeCoverageIgnoreStart
            '$1' . \PHP_Typography\U::DOUBLE_PRIME,
            '$1' . \PHP_Typography\U::SINGLE_PRIME,
        ], $node_data);
        // Backticks & comma quotes.
        $node_data = \str_replace(['``', '`', "''", ',,'], [$double_open, $single_open, $double_close, \PHP_Typography\U::DOUBLE_LOW_9_QUOTE], $node_data);
        $node_data = \preg_replace(self::COMMA_QUOTE . $f['u'], \PHP_Typography\U::SINGLE_LOW_9_QUOTE, $node_data);
        // like _,¿hola?'_.
        // Apostrophes.
        $node_data = \preg_replace([self::APOSTROPHE_WORDS . $f['u'], self::APOSTROPHE_DECADES . $f['u']], [\PHP_Typography\U::APOSTROPHE, \PHP_Typography\U::APOSTROPHE . '$1'], $node_data);
        // Quotes.
        ...
        $node_data = \str_replace(["{$double_open}{$single_open}", "{$single_close}{$double_close}"], [$double_open . \PHP_Typography\U::NO_BREAK_NARROW_SPACE . $single_open, $single_close . \PHP_Typography\U::NO_BREAK_NARROW_SPACE . $double_close], $node_data);
        ...
        $quotes = [
            ...
            $single_open,
            $single_close,
            \PHP_Typography\U::GUILLEMET_OPEN,
            // @codeCoverageIgnoreStart
            \PHP_Typography\U::GUILLEMET_CLOSE,
            \PHP_Typography\U::DOUBLE_PRIME,
            \PHP_Typography\U::SINGLE_PRIME,
            \PHP_Typography\U::APOSTROPHE,
            \PHP_Typography\U::DOUBLE_LOW_9_QUOTE,
            \PHP_Typography\U::SINGLE_LOW_9_QUOTE,
        ];
        ...
        $textnode->data = \str_replace(\PHP_Typography\RE::ESCAPE_MARKER, '', $node_data);
    }
    /**
