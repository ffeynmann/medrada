 ...
 * @since 5.0.0
 */
class Hyphenate_Fix extends \PHP_Typography\Fixes\Token_Fixes\Abstract_Token_Fix
{
    /**
     ...
     */
    public function __construct(\PHP_Typography\Hyphenator\Cache $cache = null, $target = \PHP_Typography\Fixes\Token_Fix::WORDS, $feed_compatible = \false)
    {
    ...
    {
        if (empty($settings[\PHP_Typography\Settings::HYPHENATION])) {
            return $tokens;
            // abort.
        }
        $is_heading = \false;
        if (null !== $textnode && !empty($textnode->parentNode)) {
            $block_level_parent = \PHP_Typography\DOM::get_block_parent_name($textnode);
            if (!empty($block_level_parent) && isset($this->heading_tags[$block_level_parent])) {
                $is_heading = \true;
            }
        }
        if (empty($settings[\PHP_Typography\Settings::HYPHENATE_HEADINGS]) && ($is_title || $is_heading)) {
            return $tokens;
            // abort.
        }
     ...
     */
    protected function do_hyphenate(array $tokens, \PHP_Typography\Settings $settings, $hyphen = \PHP_Typography\U::SOFT_HYPHEN)
    {
        if (empty($settings[\PHP_Typography\Settings::HYPHENATION_MIN_LENGTH]) || empty($settings[\PHP_Typography\Settings::HYPHENATION_MIN_BEFORE])) {
            return $tokens;
        }
        return $this->get_hyphenator($settings)->hyphenate($tokens, $hyphen, !empty($settings[\PHP_Typography\Settings::HYPHENATE_TITLE_CASE]), $settings[\PHP_Typography\Settings::HYPHENATION_MIN_LENGTH], $settings[\PHP_Typography\Settings::HYPHENATION_MIN_BEFORE], $settings[\PHP_Typography\Settings::HYPHENATION_MIN_AFTER]);
    }
    /**
     ...
     * @return Hyphenator
     */
    public function get_hyphenator(\PHP_Typography\Settings $settings)
    {
        $lang = $settings[\PHP_Typography\Settings::HYPHENATION_LANGUAGE];
        $exceptions = (array) $settings[\PHP_Typography\Settings::HYPHENATION_CUSTOM_EXCEPTIONS];
        $hyphenator = $this->cache->get_hyphenator($lang);
        if (null === $hyphenator) {
            $hyphenator = new \PHP_Typography\Hyphenator($lang, $exceptions);
