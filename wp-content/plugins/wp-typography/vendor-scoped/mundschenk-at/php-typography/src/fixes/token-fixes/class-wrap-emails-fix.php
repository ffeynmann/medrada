 ...
 * @since 5.0.0
 */
class Wrap_Emails_Fix extends \PHP_Typography\Fixes\Token_Fixes\Abstract_Token_Fix
{
    /**
    ...
    {
        parent::__construct(\PHP_Typography\Fixes\Token_Fix::OTHER, $feed_compatible);
        $this->email_pattern = "/(?:\n\t\t\t\\A\n\t\t\t[a-z0-9\\!\\#\$\\%\\&\\'\\*\\+\\/\\=\\?\\^\\_\\`\\{\\|\\}\\~\\-]+\n\t\t\t(?:\n\t\t\t\t\\.\n\t\t\t\t[a-z0-9\\!\\#\$\\%\\&\\'\\*\\+\\/\\=\\?\\^\\_\\`\\{\\|\\}\\~\\-]+\n\t\t\t)*\n\t\t\t@\n\t\t\t(?:\n\t\t\t\t[a-z0-9]\n\t\t\t\t[a-z0-9\\-]{0,61}\n\t\t\t\t[a-z0-9]\n\t\t\t\t\\.\n\t\t\t)+\n\t\t\t(?:\n\t\t\t\t" . \PHP_Typography\RE::top_level_domains() . '
			)
			\\Z
		)/Sxi';
    ...
    {
        if (empty($settings[\PHP_Typography\Settings::EMAIL_WRAP])) {
            return $tokens;
        }
        ...
        foreach ($tokens as $index => $token) {
            ...
            if (\preg_match($this->email_pattern, $value, $email_match)) {
                $tokens[$index] = $token->with_value(\preg_replace('/([^a-zA-Z0-9])/S', '$1' . \PHP_Typography\U::ZERO_WIDTH_SPACE, $value));
            }
        }
        return $tokens;
    }
