 ...
 * @since 5.0.0
 */
class Smart_Fractions_Fix extends \PHP_Typography\Fixes\Node_Fixes\Abstract_Node_Fix
{
    const SPACING = '/\\b(\\d+)\\s(\\d+\\s?\\/\\s?\\d+)\\b/';
    const FRACTION_MATCHING = '/
		# lookbehind assertion: makes sure we are not messing up a url
		(?<=\\A|\\s|' . \PHP_Typography\U::NO_BREAK_SPACE . '|' . \PHP_Typography\U::NO_BREAK_NARROW_SPACE . ')

		(\\d+)

		# strip out any zero-width spaces inserted by wrap_hard_hyphens
		(?:\\s?\\/\\s?' . \PHP_Typography\U::ZERO_WIDTH_SPACE . '?)
		...
		(
			...
			(?:' . \PHP_Typography\U::SINGLE_PRIME . '|' . \PHP_Typography\U::DOUBLE_PRIME . ')?

			# handle ordinals after fractions
			(?:\\<sup\\>(?:st|nd|rd|th)<\\/sup\\>)?

			# makes sure we are not messing up a url
			(?:\\Z|\\s|' . \PHP_Typography\U::NO_BREAK_SPACE . '|' . \PHP_Typography\U::NO_BREAK_NARROW_SPACE . '|\\.|,|\\!|\\?|\\)|\\;|\\:|\'|")
		)
		/Sxu';
    ...
    public function __construct($css_numerator, $css_denominator, $feed_compatible = \false)
    {
        ...
        for ($year = 1900; $year < 2100; ++$year) {
            $year_regex[] = "(?: ( {$year} ) (\\s?\\/\\s?" . \PHP_Typography\U::ZERO_WIDTH_SPACE . '?) ( ' . ($year + 1) . ' ) )';
        }
        ...
        $this->replacement = \PHP_Typography\RE::escape_tags("<sup{$numerator_css}>\$1</sup>" . \PHP_Typography\U::FRACTION_SLASH . "<sub{$denominator_css}>\$2</sub>\$3");
    }
    /**
    ...
    {
        if (empty($settings[\PHP_Typography\Settings::SMART_FRACTIONS]) && empty($settings[\PHP_Typography\Settings::FRACTION_SPACING])) {
            return;
        }
        // Cache textnode content.
        $node_data = $textnode->data;
        if (!empty($settings[\PHP_Typography\Settings::FRACTION_SPACING]) && !empty($settings[\PHP_Typography\Settings::SMART_FRACTIONS])) {
            $node_data = \preg_replace(self::SPACING, '$1' . \PHP_Typography\U::NO_BREAK_NARROW_SPACE . '$2', $node_data);
        } elseif (!empty($settings[\PHP_Typography\Settings::FRACTION_SPACING]) && empty($settings[\PHP_Typography\Settings::SMART_FRACTIONS])) {
            $node_data = \preg_replace(self::SPACING, '$1' . \PHP_Typography\U::NO_BREAK_SPACE . '$2', $node_data);
        }
        if (!empty($settings[\PHP_Typography\Settings::SMART_FRACTIONS])) {
            ...
            ], ['$1' . \PHP_Typography\RE::ESCAPE_MARKER . '$2$3', '$1' . \PHP_Typography\RE::ESCAPE_MARKER . '$2$3', $this->replacement], $node_data);
            // Unescape escaped sequences.
            $node_data = \str_replace(\PHP_Typography\RE::ESCAPE_MARKER, '', $node_data);
        }
