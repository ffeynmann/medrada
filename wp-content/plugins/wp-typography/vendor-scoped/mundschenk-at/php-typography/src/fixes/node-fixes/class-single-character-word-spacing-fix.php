 ...
 */
class Single_Character_Word_Spacing_Fix extends \PHP_Typography\Fixes\Node_Fixes\Abstract_Node_Fix
{
    const REGEX = '/
		(?:
			(\\s)
			(\\w|&)
			[' . \PHP_Typography\RE::NORMAL_SPACES . ']
			(?=\\w)
		)
	/x';
    /**
    ...
    {
        if (empty($settings[\PHP_Typography\Settings::SINGLE_CHARACTER_WORD_SPACING])) {
            return;
        }
        ...
        // Replace spaces.
        $node_data = \preg_replace(self::REGEX . $f['u'], '$1$2' . \PHP_Typography\U::NO_BREAK_SPACE, $node_data);
