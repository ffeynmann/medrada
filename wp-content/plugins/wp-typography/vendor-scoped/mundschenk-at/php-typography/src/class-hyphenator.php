...
class Hyphenator
{
    ...
    public function set_custom_exceptions(array $exceptions = [])
    {
        ...
        foreach ($exceptions as $exception) {
            $f = \PHP_Typography\Strings::functions($exception);
            if (empty($f)) {
                continue;
        ...
        if (\file_exists($language_file_name)) {
            ...
            if (\false !== $raw_language_file) {
                ...
                if (\false !== $language_file) {
                    $this->language = $lang;
                    $this->pattern_trie = \PHP_Typography\Hyphenator\Trie_Node::build_trie($language_file['patterns']);
    ...
    protected function hyphenate_word($word, $hyphen, $hyphenate_title_case, $min_length, $min_before, $min_after)
    {
        ...
        $f = \PHP_Typography\Strings::functions($word);
        if (empty($f)) {
            return $word;
            // unknown encoding, abort.
        }
